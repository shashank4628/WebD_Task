...
const filterItems = (search) => {
    filteredProducts = products.filter(prod => prod.title.toLowerCase().indexOf(search.toLowerCase()) !== -1)
    parent.innerHTML = ''
    rerender()
}
...
filteredProducts.map(d => {
    ...
    innercard.classList.add('card-body')
    title.classList.add('card-title', 'text-center')
    let subtitle = document.createElement('div')
    subtitle.classList.add('card-subtitle', 'text-center')
    subtitle.innerText = d.description
    innercard.appendChild(title)
    innercard.appendChild(subtitle)
